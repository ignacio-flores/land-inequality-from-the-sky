...
*global het_vars "gini_can dcount count_km2 count_ckm2 dherf fsize fsize_ckm2 fsize_km2 nfarms nfarms_km2 nfarms_ckm2 dct_fsize max_crop dct_km2 dct_akm2 seminat"
global het_vars "gini_can dcount count_km2 dherf fsize seminat"
...
foreach var in $het_vars {
	...
	if "`var'"=="gini_can" {
		...
		byso canton year: ereplace `var'=max(var1)
	sum `var', d
...
global whet_vars "dcount count_km2 dherf fsize seminat"

loc the_shock the_shock_${inter}
	foreach var in seminat {
		/*
	cap drop `var'_b
	gen `var'_b=`var'
	cap drop var1
...
cap drop wgini_quant_`var'
		gen wgini_quant_`var'=.
	loc q=4
	forval y=1/`q'{
		sum `var' if quant_gini_can==`y', d
		replace wgini_quant_`var'=1 if quant_gini_can==`y' & `var'<r(p25)
		replace wgini_quant_`var'=2 if quant_gini_can==`y' & `var'>=r(p25) & `var'<r(p50)
				replace wgini_quant_`var'=3 if quant_gini_can==`y' & `var'>=r(p50) & `var'<r(p75)
						replace wgini_quant_`var'=4 if quant_gini_can==`y' & `var'>=r(p75)
						sum seminat if wgini_quant_seminat==1 & quant_gini_can==`y'
						sum seminat if wgini_quant_seminat==2 & quant_gini_can==`y'
						sum seminat if wgini_quant_seminat==3 & quant_gini_can==`y'
						sum seminat if wgini_quant_seminat==4 & quant_gini_can==`y'
}
}
...
gen qrseminat=round(seminat*100,1)rseminat/5
...
foreach shock in the_shock_s canicule {
...
reghdfe gpp_w the_shock_s##i.quant_gini_can##i.wgini_quant_seminat if inrange(p, 11, 33),  a(canton##year)  vce(cl canton)
reghdfe gpp_w the_shock_s##i.quant_gini_can##i.wgini_quant_seminat hu_q if inrange(p, 11, 33),  a(canton##year)  vce(cl canton)
