...
sum fsize, d
	...
	* Normalize crop count canton-size 
	cap drop count_km2
	gen count_km2=dcount/(canton_area*area_agri)
	sum canton_area,d
		gen fs_km2=fsize/(canton_area)
				*gen dct_fsize=dcount/fsize
		sum fs_km2,d
	egen max_crop=rowmax(crop*)
sum max_crop,d

binscatter gini_can max_crop, line(connect)
binscatter fsize max_crop, line(lfit)

binscatter gini_can fsize, line(connect) n(20)
binscatter  fsize gini_can, line(connect)
binscatter  fsize gini_can, line(connect) n(20)
binscatter gini_can fsize, line(connect) 
binscatter gini_can nfarms, controls(canton_area) line(connect)

binscatter max_crop vlarge_farm_pt, line(lfit)


ta year


reg fsize crop*, cluster(canton)

reg fsize gini_can, cluster(canton)
sum gini_can,d
loc q=5
foreach var in max_crop gini_can dcount dct_fsize count_km2 dherf fsize fs_km2 {
	cap drop var1
	gen var1= `var' if year==2017
	drop `var'
...
foreach var in max_crop gini_can dcount dct_fsize count_km2 dherf fsize fs_km2 {
estimates clear
cap n	eststo `var'C: reghdfe gpp the_shockX`var' the_shock `var', absorb(canton##month t) vce(cl canton)
cap n	eststo `var'Q: reghdfe gpp t_q_`var'* q_`var'*, absorb(canton##month t) vce(cl canton)
...
scheme(plotplainblind) name(`var'Q_`y',replace);
...
preserve
estimates clear
foreach var in fsize count_km2 {
keep if year>2017 & year <2021
forval gini=2/5 {
	cap drop gini_q
	gen gini_q=q_`var'`gini'
eststo G`gini': reghdfe gpp (F*_the_shock the_shock L*_the_shock)##gini_q if (quant_`var'==1 | quant_`var'==`gini') & p>10 & p<32, absorb(canton##month t) vce(cl canton)
}
...
global xline 8.5
#delimit
coefplot(G2, m(o) mc(black) connect(line) lcolor(gs15) lpattern(solid) ciopts(lpattern(shortdash) lcolor(gs9)))
(G3, m(o) mc(black) connect(line) lcolor(gs12) lpattern(solid) ciopts(lpattern(shortdash) lcolor(gs6)))
(G4, m(o) mc(black) connect(line) lcolor(gs6) lpattern(solid) ciopts(lpattern(shortdash) lcolor(gs3)))
(G5, m(o) mc(black) connect(line) lcolor(gs0) lpattern(solid) ciopts(lpattern(shortdash) lcolor(gs0))),
keep( 1.F*_the_shock#1.gini_q 1.the_shock#1.gini_q 1.L*_the_shock#1.gini_q ) xlabel(${lab})
 yline(0, lcolor(gs0)) vertical mcolor(gs5) omitted
ciopts(recast(rline) lpattern(dash) lcolor(gs0)) ytitle("") xtitle("") xline(${xline}, lcolor(black) lpattern(shortdash))
scheme(plotplainblind) name(G`var', replace);
#delimit cr
}
...
reghdfe gpp (F*_the_shock the_shock L*_the_shock)##i.quant_gini_can, absorb(canton t) vce(cl canton)
...
cap n	eststo `var'C: reghdfe gpp the_shockX`var' the_shock gini_can if year==`y', absorb(canton t) vce(cl canton)
cap n	eststo `var'Q: reghdfe gpp t_q_`var'* q_`var'* if year==`y', absorb(canton t) vce(cl canton)
cap n	testparm t_q_`var'*, equal
cap n	estadd sca pval_id=round(r(p), .01)
...
binscatter r_gpp month if the_shock_y==0 , by(quant_gini_can) line(connect) name(binC, replace) /*ylabel(-.05(.01).02)*/
binscatter r_gpp month if the_shock_y==1 , by(quant_gini_can) line(connect) name(binT, replace) /*ylabel(-.05(.01).02)*/
...
global shock hot25
	...
	qui replace first_month = 0 if first_month == .
	so canton t 
	...
	*
	qui tab month, gen(f)
	*qui keep year canton t ${shock} agpp dinf quant_*  ///
		month f* 
	cap drop w
