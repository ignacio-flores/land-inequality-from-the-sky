...
binscatter seminat year, by(quant_fsize) discrete
cap drop lseminat
gen lseminat=ln(seminat*100)
reghdfe seminat i.quant_gini_can, vce(cl canton)
reghdfe seminat i.quant_fsize, vce(cl canton)

reghdfe seminat i.quant_gini_can##year crop1-crop28, vce(cl canton)
...
egen test=rowtotal(crop1-crop28)


sum test

binscatter seminat fsize, controls(c_crop1-c_crop5) absorb(ra_code) 
binscatter fsize gini_can, absorb(ra_code) 

binscatter dherf fsize, controls(c_crop1-c_crop5) absorb(ra_code) 
binscatter dherf  gini_can, controls(c_crop1-c_crop5)  absorb(ra_code) 


binscatter dherf year, by(quant_gini_can) controls(c_crop1-c_crop28)

binscatter seminat year, by(quant_gini_can) controls(c_crop1-c_crop28) absorb(canton)

pause on
preserve
collapse (mean) dherf seminat the_shock_s, by(canton quant_fsize year)
so canton year
xtset canton year
pause
gen lthe_shock_s=L.the_shock_s
reghdfe seminat (c.the_shock_s)##quant_fsize, a(canton)  vce(cl canton)
reghdfe dherf (c.the_shock_s)##quant_fsize, a(canton)  vce(cl canton)

restore

kdensity seminat
reg seminat i.quant_fsize, a(ra_code)  vce(cl ra_code)
reg seminat i.quant_gini_can, a(ra_code)  vce(cl ra_code)
reg dherf i.quant_fsize, a(ra_code)  vce(cl ra_code)
reg dherf i.quant_gini_can, a(ra_code)  vce(cl ra_code)


reg seminat (c.gini_can)##c.year , a(canton)  vce(cl canton)
reg seminat (c.fsize)##c.year , a(canton)  vce(cl canton)
reg fsize c.gini_can,  vce(cl canton)



preserve
collapse (mean) dherf seminat c_crop1-c_crop28 ra_code, by(canton quant_fsize year)
reg seminat i.quant_fsize##c.year,  vce(cl canton)
margins, over(quant_fsize) at(year=(2015(1)2021))  noestimcheck post coeflegend
marginsplot

preserve
collapse (mean) dherf seminat c_crop1-c_crop28 ra_code, by(canton quant_gini_can year)
reghdfe seminat i.quant_gini_can##c.year, a(ra_code##c.(c_crop1-c_crop5)) vce(cl canton)
margins, over(quant_gini_can) at(year=(2015(1)2021))  noestimcheck post coeflegend
marginsplot


preserve
collapse (mean) dherf lseminat c_crop1-c_crop28 ra_code, by(canton quant_fsize year)
reghdfe lseminat i.quant_fsize##c.year,  a(ra_code) vce(cl canton)
margins, over(quant_fsize) at(year=(2015(1)2021))  noestimcheck post coeflegend
marginsplot

preserve
collapse (mean) dherf lseminat c_crop1-c_crop28 ra_code, by(canton quant_gini_can year)
reghdfe lseminat i.quant_gini_can##c.year,  a(ra_code) vce(cl canton)
margins, over(quant_gini_can) at(year=(2015(1)2021))  noestimcheck post coeflegend
marginsplot


preserve
collapse (mean) seminat c_crop1-c_crop28 ra_code, by(canton quant_gini_can year)
reghdfe seminat i.quant_gini_can##c.year c_crop1-c_crop28, a(canton)  vce(cl canton)
margins, over(quant_gini_can) at(year=(2015(1)2021))  noestimcheck post coeflegend
marginsplot
	
forval q=1/4 {
	margins, at(year=(2015(1)2021) quant_fsize=`q') 
}
margins, over(quant_fsize) at(year=(2015(1)2021)) atmeans noestimcheck post coeflegend
marginsplot
margins, over(quant_fsize) expression((_b[_cons]+_b[2.quant_fsize]*2.quant_fsize+_b[3.quant_fsize]*3.quant_fsize +_b[4.quant_fsize]*4.quant_fsize)*c.year)  at(year=(2015(1)2020)) noestimcheck post coeflegend

margins, over(year) expression((_b[_cons]+_b[2.quant_fsize]*2.quant_fsize+_b[3.quant_fsize]*3.quant_fsize +_b[4.quant_fsize]*4.quant_fsize)) at(quant_fsize=(1(1)4)) noestimcheck post coeflegend


margins, over(quant_fsize) at(year=(2015(1)2021)) noestimcheck post coeflegend
*margins, dydx(year) at(quant_fsize=(1(1)4)) noestimcheck post coeflegend
marginsplot
margins, over(quant_fsize) at(year=(2015(6)2021)) noestimcheck post coeflegend
marginsplot
eststo marg_`var'

byso year: sum quant_fsize

margins, dydx(quant_fsize) at(year=(2016(1)2021))
reghdfe dherf i.quant_fsize##year crop1-crop28, vce(cl canton)
margins, dydx(quant_fsize) at(year=(2015(1)2021))

reghdfe i.quant_fsize##year crop1-crop28, vce(cl canton)


margins,  over(quant_fsize) at(year=(2015(1)2021))
marginsplot

reghdfe seminat quant_fsize##i.year, a(canton) vce(cl canton)
margins, over(quant_fsize) at(year=(2015(1)2021))
margins, dydx(quant_fsize) at(year=(2015(1)2021))
marginsplot
xtset canton t
...
binsreg gpp_${inter} tsup_h_q, by(quant_gini_can) absorb(canton year) samebinsby  usegtools(on) ///
...
binsreg gpp_${inter} tsup_h_q, by(quant_fsize) absorb(canton year) samebinsby  usegtools(on) ///
savedata(data/FR/aux_files/bin_data1) replace xtitle(Temperature °C) ytitle(GPP in C.kg/m{superscript:2} (weekly)) ///
...
gen int cutf=.
forval q=1/4 {
sum tsup_h_q if the_shock_s & quant_fsize==`q',
global cutf=r(min)
replace cutf=round(${cutf}) if quant_fsize==`q'
}
sum cut
loc cut=r(mean)
*se trace on
binsreg gpp_${inter} tsup_h_q if tsup_h_q>=cut, by(quant_gini_can) absorb(canton##year) samebinsby polyreg(3) usegtools(on) ///
...
sum cut
loc cut=r(mean)
binsreg gpp_${inter} tsup_h_q if tsup_h_q>=cutf, by(quant_fsize) absorb(canton##year) samebinsby polyreg(3)  usegtools(on) ///
savedata(data/FR/aux_files/bin_data_zoom`cut') replace xtitle(Temperature °C) ytitle(GPP in C.kg/m{superscript:2} (weekly)) ///
	...
	foreach qtle in `quant' {
			...
			*/
			...
			forval q=2/`qtle' {
				...
				if (quant_`var'==1 | quant_`var'==`q') & tsup_h_q_r<35, absorb(canton year) vce(cl canton) resid
...
forval q=1/`g' {
did_multiplegt_dyn lgpp_${inter} can_year p the_shock_s if inrange(`range') & quant_`var'==`q', placebo(`nump') effects(`nume') trends_nonparam(canton) `switchers'
		matrix C`q' = J(`num',3,.)
loc i `nump'
forval p=1/`nump'{
...
(matrix(C4[,1]), rename(r2=q4) ci((C4[,2] C4[,3])) m(S) mc(dkorange)  ciopts(lpattern(shortdash) lcolor(dkorange))), drop(r1)
				yline(0, lcolor(gs0)) vertical ylabel(${ylab})
				ciopts(/*recast(rline) */lpattern(dash)) 
				ytitle("") xtitle("") scheme(plotplainblind) legend(off) name(ATE_mult_`var', replace) ;
...
forval q=1/`g' {
did_multiplegt_dyn lgpp_${inter} can_year p the_shock_s if inrange(`range') & quant_`var'==`q', effects(`nume') placebo(`nump')  trends_nonparam(canton)
...
(matrix(D4[,1]),  ci((D4[,2] D4[,3])) m(S) mc(dkorange)  ciopts(lpattern(shortdash) lcolor(dkorange))),
				...
				ytitle("") xtitle("") scheme(plotplainblind) legend(order(2 "q1" 4 "q2" 6 "q3" 8 "q4" ) pos(0) bplace(sw) ) name(trend_mult_`var', replace) ;
