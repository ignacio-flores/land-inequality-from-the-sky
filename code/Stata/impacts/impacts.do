...
global DG "C:/Users/dylbo/Dropbox/land_ineq_degradation/"
global rep $DG
cd $DG
u data/FR/cadastral_panel, clear
...
set matsize 5000
* Looking at relationship between inequality and %agriculture
* Use residualization based on Cattaneo et al. (2019)
...
graph export "figures/descriptives//CEF_gini_area.pdf", replace ;
#delimit cr
...
plotregion(style(none)) plotregion(style(none)) graphregion(fcolor(white));
graph export "figures/descriptives//CEF_gini_area_cond.pdf", replace ;
#delimit cr
...
plotregion(style(none)) plotregion(style(none)) graphregion(fcolor(white));
graph export "figures/descriptives//CEF_NPP_area.pdf", replace ;
#delimit cr
...
plotregion(style(none)) plotregion(style(none)) graphregion(fcolor(white));
graph export "figures/descriptives//CEF_NPP_area_cond.pdf", replace ;
#delimit cr
...
plotregion(style(none)) plotregion(style(none)) graphregion(fcolor(white))
graph export "figures/descriptives//Corr_crop_gini.pdf", replace
...
#delimit
esttab A0 A B C using "figures/descriptives/all_`xvar'.tex",  f nonum nomti noli 
...
#delimit
esttab `xvar'q using "figures/descriptives//het_agri_impact_`xvar'.tex",  f nonum nomti noli 
...
foreach xvar in gini_can if qarea>1 {
eststo A0: reg mean_ion_npp `xvar',   vce(cl canton)
...
#delimit
esttab A0 A B C using "figures/descriptives/all_`xvar'_exclq1.tex",  f nonum nomti noli 
collabels(none) keep(`xvar') `stats' style(tex) noobs cells(b(fmt(4) star) 
se(par fmt(4))) star(* .1 ** .05 *** .01)  replace;
#delimit cr
}
...
plotregion(style(none)) plotregion(style(none)) graphregion(fcolor(white))
graph export "figures/descriptives//CEF_NPP_gini_cond_exclq1.pdf", replace 
...
plotregion(style(none)) plotregion(style(none)) graphregion(fcolor(white));
graph export "figures/descriptives//npp_gini_trend.pdf", replace ;
#delimit cr

* Consistent with Fuzzy difference in differences à la de Chaisemartin et D’Haultfœuille?
 did_multiplegt mean_ion_npp canton year treatment if qarea>1, placebo(3) breps(10) cluster(canton) dynamic(2) 
ereturn list
