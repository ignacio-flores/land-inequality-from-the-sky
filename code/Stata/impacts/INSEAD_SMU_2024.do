...
global het_vars "gini_can dcount count_km2 count_ckm2 dherf fsize fsize_ckm2 fsize_km2 nfarms nfarms_km2 nfarms_ckm2 dct_fsize max_crop dct_km2 dct_akm2"
...
sum cgpp_${inter} if p==pmax ,d

binsreg cgpp_${inter}_p gini_can if p==pmax & gini_can<=cut_gini_can, absorb(ndept##year) line(1 1) xtitle(Land Gini) ytitle(Annual GPP in C.Mt/ha) ///
scheme(plotplainblind) name(gini_can , replace)
graph export "$figs/food_agg_prod_by_gini.png", replace 
binsreg cgpp_${inter}_p fsize  if p==pmax & fsize<=cut_fsize, absorb(ndept##year) line(1 1) xtitle(Avg Farm Size in ha) ytitle(Annual GPP in C.Mt/ha) ///
...
foreach var in gini_can fsize {
	...
	forval x= 1/`q' {
		...
		cap n	eststo `var'Q: reghdfe gpp_${inter} t_q_`var'? if inrange(p, 11, 33), absorb(canton#year) vce(cl canton)
		...
		#delimit
			coefplot `var'Q , drop(_cons)
				...
				name(`var'Q,replace);
			graph export "$figs/het_impact_`var'.png", replace ;
		#delimit cr
}
